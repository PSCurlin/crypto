/**
 * @file sha512.c
 * @brief Demontration of SHA512 hashing.
 */
...
void hash(const char *msg, unsigned char digest[SHA512_DIGEST_LENGTH]);
...
void hash(const char *msg, unsigned char digest[SHA512_DIGEST_LENGTH]) {
    ...
    if(1 != EVP_DigestInit_ex(ctx, EVP_sha512(), NULL)) handleErrors();
...
int main(void) {
    ...
    printf("\n");
    BIO_dump_fp(stdout, (const char *)digest, SHA512_DIGEST_LENGTH);
    printf("\n");
