/**
 * @file sha3_384.c
 * @brief Demontration of SHA3-384 hashing.
 */
...
void hash(const char *msg, unsigned char digest[SHA384_DIGEST_LENGTH]);
...
void hash(const char *msg, unsigned char digest[SHA384_DIGEST_LENGTH]) {
    ...
    if(1 != EVP_DigestInit_ex(ctx, EVP_sha3_384(), NULL)) handleErrors();
...
int main(void) {
    ...
    printf("-> Will run SHA3-384 hashing.\n");
    ...
    printf("\n");
    BIO_dump_fp(stdout, (const char *)digest, SHA384_DIGEST_LENGTH);
    printf("\n");
